...
<div class="breadcrumb-header justify-content-between">
    <div class="left-content">
        <span class="main-content-title mg-b-0 mg-b-lg-1"><?=$heading?>(<?=$Brandcount;?>)</span>
    </div>
    ...
    <div class="col-12 col-sm-12">
        <div class="card">
            <div class="card-header">
                <h4 class="card-title"><?=$heading?> <a href="<?=admin_url('add-brand');?>">Add New Brand</a></h4>
            </div>
                ...
                <form action="" id="data_form" method="Post">
                    ...
                    <table class="table  table-bordered text-nowrap mb-0" id="example1">
                        ...
                        <?php 
                           if ($BrandRes) {
                $i=1;
                            foreach($BrandRes as $key =>  $pageVal){ 
                                $displayorder = ($pageVal['sort_order'] != '') ? $pageVal['sort_order'] : "0";
                              ?>
                            <tr>
                                <td class="text-center">
                                    <label class="custom-control custom-checkbox">
                                        <input type="checkbox" class="custom-control-input" name="arr_ids[]" value="<?php echo $pageVal['brand_id']; ?>" >
                                        <span class="custom-control-label custom-control-label-md  tx-17"></span>
                                    </label>
                                    </td>
                                <td>
                                <?php 
                            echo $pageVal['brand_name'];
                            ?>
                                </td>
                                
                                <td class="text-center">
                                <img src="<?=base_url('public/upload/brand/'.$pageVal['brand_img']);?>" title="<?=$pageVal['brand_name']?>" alt="<?=$pageVal['brand_name']?>" class="imagethumb">
                                </td>
                                <td><input type="tel" class="form-control order_input" name="ord[<?php echo $pageVal['brand_id']; ?>]" value="<?=$displayorder;?>"  onkeyup="this.value=this.value.replace(/[^0-9]/g,'')" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" maxlength="2"></td>
                            ...
                            ?>                 
                                ...
                                <td>
                            <a href="<?=admin_url('edit-brand')."/".$pageVal['brand_id'];?>" class="reply_btn" data-bs-toggle="tooltip" data-bs-original-title="Edit"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16"><path d="M12.146.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1 0 .708l-10 10a.5.5 0 0 1-.168.11l-5 2a.5.5 0 0 1-.65-.65l2-5a.5.5 0 0 1 .11-.168l10-10zM11.207 2.5 13.5 4.793 14.793 3.5 12.5 1.207 11.207 2.5zm1.586 3L10.5 3.207 4 9.707V10h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.293l6.5-6.5zm-9.761 5.175-.106.106-1.528 3.821 3.821-1.528.106-.106A.5.5 0 0 1 5 12.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.468-.325z"/></svg></a>
                            </td>                                            
                            </tr>
                            <?php } } else{ ?>
<tr class="text-center">
    <td colspan='6'>No Brand Details Found</td>
</tr>
                          <?php  } ?>
                        </tbody>
                    </table>
